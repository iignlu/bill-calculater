document.getElementById("calcBtn").addEventListener("click", () => {
  ...
  // --- CALCULATIONS (using readable variable names) ---
  ...
  // --- OUTPUTS with readable formulas ---
  const outputs = [
    ["Reactive_kVArh_old", Reactive_kVArh_old, "E_kWh × TAN(ACOS(PF_old))"],
    ["Reactive_kVArh_new", Reactive_kVArh_new, "E_kWh × TAN(ACOS(PF_new))"],
    ["Allowed_kVArh", Allowed_kVArh, "E_kWh × AllowedReactiveRatio"],
    [
      "Excess_kVArh_old",
      Excess_kVArh_old,
      "MAX(0, Reactive_kVArh_old − Allowed_kVArh)",
    ],
    [
      "Excess_kVArh_new",
      Excess_kVArh_new,
      "MAX(0, Reactive_kVArh_new − Allowed_kVArh)",
    ],
    ["Penalty_old_SAR", Penalty_old_SAR, "PenaltyRate × Excess_kVArh_old"],
    ["Penalty_new_SAR", Penalty_new_SAR, "PenaltyRate × Excess_kVArh_new"],
    [
      "Penalty_savings_SAR",
      Penalty_savings_SAR,
      "Penalty_old_SAR − Penalty_new_SAR",
    ],
    [
      "CurrentFactor_old",
      CurrentFactor_old,
      "SQRT(1 + THDi_old²) ÷ PF_old",
    ],
    [
      "CurrentFactor_new",
      CurrentFactor_new,
      "SQRT(1 + THDi_new²) ÷ PF_new",
    ],
    [
      "LossRatio",
      LossRatio,
      "(CurrentFactor_new ÷ CurrentFactor_old)²",
    ],
    [
      "kWh_saved_copper",
      kWh_saved_copper,
      "E_kWh × BaseLossFrac × (1 − LossRatio)",
    ],
    [
      "Bank_losses_kWh",
      Bank_losses_kWh,
      "(Bank_kVAr_avg × Bank_loss_W_per_kVAr × Hours) ÷ 1000",
    ],
    ["SAPF_losses_kWh", SAPF_losses_kWh, "SAPF_loss_kW × Hours"],
    [
      "kWh_saved_net",
      kWh_saved_net,
      "kWh_saved_copper − Bank_losses_kWh − SAPF_losses_kWh",
    ],
    [
      "Energy_savings_SAR",
      Energy_savings_SAR,
      "EnergyRate × kWh_saved_net",
    ],
    ["PF_true_old", PF_true_old, "PF_old ÷ SQRT(1 + THDi_old²)"],
    ["PF_true_new", PF_true_new, "PF_new ÷ SQRT(1 + THDi_new²)"],
    [
      "S_old_kVA",
      S_old_kVA,
      "IF(DemandBilledInKVA=1, Peak_kW ÷ PF_true_old, 0)",
    ],
    [
      "S_new_kVA",
      S_new_kVA,
      "IF(DemandBilledInKVA=1, Peak_kW ÷ PF_true_new, 0)",
    ],
    [
      "Demand_savings_SAR",
      Demand_savings_SAR,
      "(S_old_kVA − S_new_kVA) × DemandRate",
    ],
    [
      "Total_monthly_savings_SAR",
      Total_monthly_savings_SAR,
      "Penalty_savings_SAR + Energy_savings_SAR + Demand_savings_SAR",
    ],
    [
      ...
      Simple_payback_years,
      "IF(Total_monthly_savings_SAR>0, CAPEX ÷ (12×Total_monthly_savings_SAR), '')",
    ],
  ];
  ...
  outputs.forEach(([label, value, formula]) => {
